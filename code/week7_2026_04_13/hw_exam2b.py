...
for i in range(8):
    for j in range(8):#第一次:一次函數,第二次:二次函數,第三次:三次函數...以此類推 第i次: j次函數(j=1,2,...,i)
        if j != i:
            ...
            validlist = []
            i_j.append((i, j))
            for k in range(8):#第一次:一次函數,第二次:二次函數,第三次:三次函數...以此類推 第i次: j次函數(j=1,2,...,i)
                if k != i and k != j:
                    #print("i:", i, "j:", j, "k:", k)
                    trainlist.append(trainX[:,k])
                    trainx = np.transpose(trainlist)
                
                
                    validlist.append(validX[:, k])#第一次:一次函數,第二次:二次函數,第三次:三次函數...以此類推 第i次: j次函數(j=1,2,...,i)
...
for i in range(8 * 7):
    print("i_j:", i_j[i], "logvalidloss:", logvalidloss[i], end='\n')

print("\n\n### results of removing two features ###\n")

min_logvalidloss = min(logvalidloss)
print("min logvalidloss:", min_logvalidloss, end='\n\n')     
min_logvalidloss_index = logvalidloss.index(min_logvalidloss)
print("index of min logvalidloss:", min_logvalidloss_index, end='\n\n')
print("removed feature index:", i_j[min_logvalidloss_index][0], "removed feature name:", df.columns[i_j[min_logvalidloss_index][0]], "\nremoved feature index:", i_j[min_logvalidloss_index][1],"removed feature name:", df.columns[i_j[min_logvalidloss_index][1]],  end='\n\n')

print("### end ###\n\n")

"""
ran = len(trainloss) # total number of combinations of features
plt.plot(range(len(trainloss)), trainloss, marker='o', label='Train Loss')
...
plt.plot(range(ran), logvalidloss, marker='o', label='Log Validation Loss')
